...
if ! fetch_to_file "$ARCHIVE_URL" "$ARCHIVE_FILE"; then
    ...
    echo "  /bin/uclient-fetch --no-check-certificate -O - https://raw.githubusercontent.com/dagmagnat/RouteWolf/main/install.sh | sh"
...
if [ -r /dev/tty ]; then
    sh ./routewolf-install.sh "$@" < /dev/tty
    RC="$?"
else
    sh ./routewolf-install.sh "$@"
    RC="$?"
fi
